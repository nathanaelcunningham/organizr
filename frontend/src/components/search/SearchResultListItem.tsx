...
export const SearchResultListItem: React.FC<SearchResultListItemProps> = ({
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const series = result.series && result.series.length > 0 ? result.series[0].name : ''
      const seriesNumber = result.series && result.series.length > 0 ? result.series[0].number : ''
      ...
      >
          ...
          <div className="min-w-0 grow">
            <h3 className="text-sm font-semibold text-gray-900 truncate">
              {showSeriesNumber && result.series && result.series.length > 0 && (
                <span className="text-gray-500 font-normal mr-2">#{result.series[0].number}</span>
              )}
              {result.title}
            </h3>
            <p className="text-xs text-gray-600 truncate mt-0.5">by {result.author}</p>
          </div>
        ...
        <div className="hidden md:flex md:items-center md:gap-3 md:shrink-0">
          ...
          <div className="w-20 text-right">
            <span className="text-sm font-medium text-gray-900">{formatFileSize(result.size)}</span>
          </div>
          ...
          <div className="space-y-3">
            ...
            {((result.series && result.series.length > 0) || result.narrator) && (
              <div className="text-sm">
                ...
                {result.narrator && (
                  <p className="text-gray-700">
                    <span className="font-medium">Narrated by:</span> {result.narrator}
                  </p>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
              ...
              {result.num_files && (
                <div>
                  ...
                  <span className="ml-2 font-medium text-gray-900">{result.num_files}</span>
                </div>
              )}
              {result.times_completed && (
                <div>
                  <span className="text-gray-500">Downloads:</span>
                  <span className="ml-2 font-medium text-gray-900">{result.times_completed}</span>
                </div>
              )}
              {result.added && (
                <div>
                  <span className="text-gray-500">Added:</span>
                  <span className="ml-2 font-medium text-gray-900">{result.added}</span>
                </div>
              )}
            </div>
