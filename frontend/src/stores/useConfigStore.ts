...
export const useConfigStore = create<ConfigStore>((set, get) => ({
  ...
  fetchConfig: async () => {
    try {
      ...
      const message =
        error instanceof APIClientError ? error.message : 'Failed to fetch configuration'
  ...
  updateConfig: async (key: string, value: string) => {
    try {
      ...
      const message =
        error instanceof APIClientError ? error.message : 'Failed to update configuration'
      useNotificationStore.getState().addNotification('error', message)
      return false
    }
  },
  ...
  updateMultipleConfigs: async (updates: Record<string, string>) => {
    try {
      ...
      const results = await Promise.allSettled(
        Object.entries(updates).map(([key, value]) => configApi.update(key, { value }))
      )
