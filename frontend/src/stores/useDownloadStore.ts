...
interface DownloadStore {
  ...
  ) => Promise<{ successful: Download[]; failed: BatchDownloadError[] }>
...
export const useDownloadStore = create<DownloadStore>((set, get) => ({
  ...
  fetchDownloads: async () => {
    try {
      ...
      const message = error instanceof APIClientError ? error.message : 'Failed to fetch downloads'
  ...
  createDownload: async (data: CreateDownloadRequest) => {
    try {
      ...
      useNotificationStore.getState().addNotification('success', 'Download started successfully')
      ...
      const message = error instanceof APIClientError ? error.message : 'Failed to create download'
      useNotificationStore.getState().addNotification('error', message)
      return undefined
    }
  },
    ...
    try {
      ...
      if (failCount === 0) {
        ...
        useNotificationStore
          ...
          .addNotification(
            'success',
            `${successCount} download${successCount === 1 ? '' : 's'} started successfully`
          )
        ...
        useNotificationStore
          ...
          .addNotification(
            'warning',
            `${successCount} download${successCount === 1 ? '' : 's'} started, ${failCount} failed`
          )
      }
      ...
      const message =
        error instanceof APIClientError ? error.message : 'Failed to create batch downloads'
      useNotificationStore.getState().addNotification('error', message)
  ...
  cancelDownload: async (id: string) => {
    try {
      ...
      useNotificationStore.getState().addNotification('success', 'Download cancelled')
    } catch (error) {
      const message = error instanceof APIClientError ? error.message : 'Failed to cancel download'
      useNotificationStore.getState().addNotification('error', message)
    }
  },
  ...
  organizeDownload: async (id: string) => {
    try {
      ...
      useNotificationStore.getState().addNotification('success', 'Organizing download...')
      // Fetch updated status
      await get().fetchDownloads()
    } catch (error) {
      const message =
        error instanceof APIClientError ? error.message : 'Failed to organize download'
      useNotificationStore.getState().addNotification('error', message)
    }
  },
  ...
  // Computed getters
  getActiveDownloads: () =>
    get().downloads.filter((d) => ['queued', 'downloading', 'organizing'].includes(d.status)),

  getCompletedDownloads: () => get().downloads.filter((d) => d.status === 'completed'),

  getFailedDownloads: () => get().downloads.filter((d) => d.status === 'failed'),
}))
